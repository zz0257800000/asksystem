...
export default {
    ...
    methods: {
        ...
        postToDB() {
            if (!this.questionName  || !this.startTime || !this.endTime) {
        alert("請填寫所有必填項目");
        return; // 不進行POST請求和跳轉
    ...
    <div class="createQuestPageBody">
        ...
        <div class="createQuest" v-for="(quest, questionIndex) in questArr" :key="questionIndex">
            ...
            <div class="NewOptions" v-for="(option, optionIndex) in quest.options" :key="optionIndex">
                ...
                <button style="background-color: rgb(0, 0, 0);color: red;" @click="deleteNewOptions(questionIndex, optionIndex)">刪除選項</button>
            </div>

            <button style="margin-left: 43px; background-color: rgb(0, 0, 0); color: red;"
